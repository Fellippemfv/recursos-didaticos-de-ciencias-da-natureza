...
const Navbar = () => {
  ...
  return (
    <ul className="hidden sm:flex items-center space-x-4 leading-5 sm:space-x-6">
      ...
      <NavItem icon={<FcSearch />} text="Pesquisar" path="/search" />
      <NavItem icon={<FcInfo />} text="Sobre" path="/about" />
    </ul>
  );
};
  ...
  <li className="nav-item">
    <Link href={path}>
      <div className="relative flex items-center py-2 pr-2 border-b-2 border-transparent hover:border-gray-300">
        {icon}
        <span className="ml-2 text-base leading-6 font-medium text-secondary-500">
          {text}
        </span>
      </div>
    </Link>
